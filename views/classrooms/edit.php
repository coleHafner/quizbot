<h1><?php echo $room->isNew() ? "New" : "Edit" ?> Classroom</h1>
<form method="post" action="<?php echo site_url('classrooms/save') ?>">
	<div class="ui-widget-content ui-corner-all ui-helper-clearfix">
		<input type="hidden" name="id" value="<?php echo h($room->getId()) ?>" />

		<div class="form-field-wrapper">
			<label class="form-field-label" for="classroom_name">Name</label>
			<input id="classroom_name" type="text" name="name" value="<?php echo h($room->getName()) ?>" />
		</div>
		...
		<div class="form-field-wrapper">
				...
				<select name="teacher_id">
					...
					<?php foreach ($teachers as $teacher) : ?>
						<option value="<?= $teacher->getId(); ?>"
							<?php echo $teacher->hasClassroom($room) ? 'selected' : ''; ?>>
							<?= $teacher ?>
						</option>
					<?php endforeach; ?>
				</select>
			...
			<?php else : ?>
				<?php $teacher = $room->isNew() ? App::getUser() : $room->getTeacher(); ?>
				<?= $teacher; ?>
	...
	<div class="form-action-buttons ui-helper-clearfix">
		<span class="button" data-icon="disk">
			<input type="submit" value="<?php echo $room->isNew() ? "Save" : "Save Changes" ?>" />
		</span>
